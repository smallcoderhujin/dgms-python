...
class MountCdromFailed(NovaException):
    msg_fmt = _("Unable to mount/unmount cdrom to instance %(instance)s, error: %(error)s")


class MultiplePortsFound(Invalid):
    msg_fmt = _("Multiple Ports object were be found by one port id %(port_id)s.")


class SetFixedIpFail(Invalid):
    msg_fmt = _("Unable to update port %(port_id)s of instance %(instance_id)s"
                " with failure: %(error)s.")
